...
static const GPIO_TYPE __CLK = {
	GPIOA, {GPIO_Pin_0, GPIO_Speed_50MHz, GPIO_Mode_AF_PP}, RESET,
};
...
/*******************************************************************************
 * Function Name  : SPI_CreateDefaultSPI
 * Description    : 发送数据，以及读取数据
 * Input          : dat(需要发送的数据) 
 * Output         : None
 * Return         : 读取到的数据
 *******************************************************************************/
void SPI_CreateDefaultSPI(SPI_TYPE *me) {
	if (me->SPI_MODE == SPI_MODE_INSIDE) {
	   if ((SPI_TypeDef *)(me->SPIx) == SPI1) { // SPI1
		...
		me->GPIO_CS.GPIOInitTypeStruct.GPIO_Mode = GPIO_Mode_Out_PP;
		me->GPIO_CS.GPIOInitTypeStruct.GPIO_Speed = GPIO_Speed_50MHz;
		me->GPIO_CS.IntValue = RESET;

		me->GPIO_CLK.GPIOInitTypeStruct.GPIO_Mode = GPIO_Mode_Out_PP;
		me->GPIO_CLK.GPIOInitTypeStruct.GPIO_Speed = GPIO_Speed_50MHz;
		
		me->GPIO_MISO.GPIOInitTypeStruct.GPIO_Mode = GPIO_Mode_IN_FLOATING;
		//me->GPIO_MISO.GPIOInitTypeStruct.GPIO_Speed = GPIO_Speed_50MHz;
		me->GPIO_MISO.IntValue = RESET;
		
		me->GPIO_MOSI.GPIOInitTypeStruct.GPIO_Mode = GPIO_Mode_Out_PP;
		me->GPIO_MOSI.GPIOInitTypeStruct.GPIO_Speed = GPIO_Speed_50MHz;
		me->GPIO_MOSI.IntValue = RESET;
